...
echo "Building bab_rs for WebAssembly..."
...
wasm-pack build --target "$TARGET" --features wasm,william3
...
if [ "$TARGET" = "web" ]; then
    ...
    echo "  import init, { william3_hash, William3HasherWasm } from './pkg/bab_rs.js';"
    echo "  await init();"
    echo "  const hash = william3_hash(new TextEncoder().encode('hello world'));"
elif [ "$TARGET" = "nodejs" ]; then
    echo "Usage in Node.js:"
    echo "  import { william3_hash, William3HasherWasm } from './pkg/bab_rs.js';"
    echo "  const hash = william3_hash(new TextEncoder().encode('hello world'));"
elif [ "$TARGET" = "bundler" ]; then
    echo "Usage with bundlers (webpack, rollup, etc.):"
    echo "  import init, { william3_hash, William3HasherWasm } from 'bab_rs';"
    echo "  await init();"
    echo "  const hash = william3_hash(new TextEncoder().encode('hello world'));"
fi
